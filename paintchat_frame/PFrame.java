	       ...
	       Runnable
{
	...
	if (actionevent.getSource() == getPaintchat_Port()) {
	    ...
	    getPaintchat_Port().setText(string);
	    config.put("Connection_Port_PaintChat", string);
	...
	if (actionevent.getSource() == getHttp_Port()) {
	    ...
	    getHttp_Port().setText(string);
	    config.put("Connection_Port_Http", string);
	    config.saveConfig(null, null);
	}
    }
    ...
    public void connEtoC18_NormalResult(boolean bool) {
	try {
	    if (bool)
		new ConfigDialog
		    ("paintchat.config.PConfig", "cnf/dialogs.jar", config,
		     res,
		     "(C)\u3057\u3043\u3061\u3083\u3093 PaintChatApp v3.66");
	...
	try {
	    getHelp().startHelp
		(new HelpWindowContent(mouseevent.getComponent().getName(),
				       true, Gui.getScreenPos(mouseevent),
				       res));
	} catch (Throwable throwable) {
	    handleException(throwable);
	}
	...
	if (ivjConsole == null) {
		...
		applet.setStub(ServerStub.getDefaultStub(getConfig(),
							 getResource()));
		ivjConsole.init(applet, 400, ivjConsole.getBackground(),
				ivjConsole.getForeground(), null);
	    } catch (Throwable throwable) {
		handleException(throwable);
	    }
	}
	...
	if (ivjPanelLeft == null) {
		...
		getPanelLeft().add(getPaintchat_Port(),
				   getPaintchat_Port().getName());
		getPanelLeft().add(getPaintchat_Button(),
				   getPaintchat_Button().getName());
		getPanelLeft().add(getHttp_Button(),
				   getHttp_Button().getName());
		getPanelLeft().add(getLobby_Button(),
				   getLobby_Button().getName());
	    } catch (Throwable throwable) {
		handleException(throwable);
	    }
	}
	...
	if (ivjPFrameMenuBar == null) {
		...
		for (int i_0_ = 0; i_0_ < i; i_0_++)
		    setResource(menubar.getMenu(i_0_));
	    } catch (Throwable throwable) {
		handleException(throwable);
	    }
	}
	...
	PopupMenuPaintChat popupmenupaintchat
	    = new PopupMenuPaintChat(debug, config, res);
	...
	try {
	    new ConfigDialog
		("paintchat.config.ConfigServer", "cnf/dialogs.jar", config,
		 res, "(C)\u3057\u3043\u3061\u3083\u3093 PaintChatApp v3.66");
	} catch (Exception exception) {
	    exception.printStackTrace();
	}
    }
	...
	if (getData().isRunPaintChatServer()
	    && (MessageBox.confirm
		("ConfirmMayServerStopNow",
		 "(C)\u3057\u3043\u3061\u3083\u3093 PaintChatApp v3.66")))
	...
	try {
	    new ConfigDialog
		("paintchat.config.PConfig", "cnf/dialogs.jar", config, res,
		 "(C)\u3057\u3043\u3061\u3083\u3093 PaintChatApp v3.66");
	} catch (Throwable throwable) {
	...
	stringbuffer
	    .append("(C)\u3057\u3043\u3061\u3083\u3093 PaintChatApp v3.66");
	stringbuffer.append('\n');
	stringbuffer.append('\n');
	...
	try {
	    config.put("Connection_Port_Http",
		       getHttp_Port().getText().trim());
	    config.put("Connection_Port_PaintChat",
		       getPaintchat_Port().getText().trim());
	    config.saveConfig(null, null);
	    if (getData().getIsNativeWindows())
		getData().exitWin();
	    this.dispose();
	...
	try {
	    ...
	    getConfig();
	    getHttp_Port().setText(config.getString("Connection_Port_Http",
						    "80"));
	    getPaintchat_Port()
		.setText(config.getString("Connection_Port_PaintChat", "0"));
	    Image image = Io.loadImageNow(this, "cnf/template/top.jpg");
	    Dimension dimension = this.getToolkit().getScreenSize();
	    Point point
		= new Point((dimension.width - image.getWidth(null)) / 2,
			    (dimension.height - image.getHeight(null)) / 2);
		...
		showHelp();
	    if (!config.getString("App_Version").equals
		 ("(C)\u3057\u3043\u3061\u3083\u3093 PaintChatApp v3.66")) {
	    ...
	    getData().init(config, res, debug, getPaintchat_Button(),
			   getHttp_Button(), getLobby_Button());
	    this.pack();
	    Awt.moveCenter(this);
	...
	menuitem.setLabel(getResource().get(menuitem.getLabel(),
					    menuitem.getLabel()));
	if (menuitem instanceof Menu) {
	    Menu menu = (Menu) menuitem;
	    int i = menu.getItemCount();
	    for (int i_1_ = 0; i_1_ < i; i_1_++)
		setResource(menu.getItem(i_1_));
	}
    }
    
    public void setResource(Res res, Component component) {
	if (component instanceof Container) {
	    Container container = (Container) component;
	    int i = container.getComponentCount();
	    for (int i_2_ = 0; i_2_ < i; i_2_++)
		setResource(res, container.getComponent(i_2_));
	} else {
	    String string = res.get(component.getName(), component.getName());
	    if (component instanceof LTextField) {
		LTextField ltextfield = (LTextField) component;
		ltextfield.setTitle(string);
	...
	boolean bool = console.getParent() == this;
	if (bool) {
	    this.remove(console);
	    console.stop();
	    MenuBar menubar = this.getMenuBar();
	    ...
	    MenuBar menubar = getPFrameMenuBar();
	    menubar.add(getMenu1());
	    menubar.add(getMenu3());
	}
	config.put("App_IsConsole", String.valueOf(!bool));
	ivjMenuShowConsole.setState(!bool);
	this.pack();
    }
	...
	config.put("App_ShowHelp", String.valueOf(!bool));
    }
    
    public void startClient() {
	try {
	    AppletWatcher appletwatcher
		= (new AppletWatcher
		   ("paintchat_client.Client",
		    "(C)\u3057\u3043\u3061\u3083\u3093 PaintChatApp v3.66",
		    config, res, false));
	    appletwatcher.setIconImage(this.getIconImage());
	    appletwatcher.show();
	} catch (Throwable throwable) {
	    debug.log(throwable.getMessage());
	}
